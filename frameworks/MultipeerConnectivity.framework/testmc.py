...
if __name__ == '__main__':
    ...
    # custom test parser
    parser_custom 		= subparsers.add_parser('custom',   help='Run individual tests.', parents=[parser_parent])

    parser_custom.add_argument("testName", choices=MCGetTestNames(), help="Name test to run.")


    # presub parser
    parser_presub 		= subparsers.add_parser('presub',   help='Run presubmission test.', parents=[parser_parent])
    
    parser_presub.add_argument("-r", "--runs",              default=1,
                                                            help="Number of runs.")
                                                            
    # systempass parser
    syspass_presub 		= subparsers.add_parser('systempass',
                                                            help='Run system pass test.', parents=[parser_parent])
    
    args            = parser.parse_args()
    command         = args.command
    wifi_network	= args.wifi_network
